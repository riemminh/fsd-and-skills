#!/usr/bin/env bash
# Copy upstream: https://github.com/rtk-ai/rtk/blob/develop/hooks/cursor/rtk-rewrite.sh
# rtk-hook-version: 1
...
if [ -n "$RTK_VERSION" ]; then
  ...
  if [ "$MAJOR" -eq 0 ] && [ "$MINOR" -lt 23 ]; then
    echo "[rtk] WARNING: rtk $RTK_VERSION is too old (need >= 0.23.0). Upgrade: https://github.com/rtk-ai/rtk#installation" >&2
    exit 0
  fi
fi
